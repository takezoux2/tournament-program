...
（`carryMatchNames(previous, next, firstRoundIdMap?)`）。
...
- 組み合わせが league 形状のまま（shape mismatch）、またはエントリーはあるが組み合わせが空:
...
- アイコンはインライン SVG（アイコンライブラリは入れない）。`aria-label="選手を編集"`。
...
- 同じメンバーが既に部門内のスロットにいれば `assign-slot` は拒否。
