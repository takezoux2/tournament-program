...
`entries` / `matchingConfig` / `results` は**読み取りのみ**。詳細ページの表示に使うだけで、
...
隣が存在しない要求（端まで来ている場合と、その大会にその部門が無い場合の両方）は
`{ swapped: false }` を返す。端まで来ているのはエラーではなく、かつこの 2 つの
ケースは応答上区別できない（区別すると存在を漏らすことになる）ため、ハンドラは
どちらも `notFound()` にはせず、同じ `{ error: null }` の no-op として扱う。
並べ替えは大会詳細ページに留まる操作であり、redirect も行わない。
