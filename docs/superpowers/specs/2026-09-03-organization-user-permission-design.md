...
4. 追加されたユーザーには**追加者が持つ Permission だけを付与**する
   （2026-09-03 の決定で「全 Permission を付与」から変更。自分より強いメンバーを
   作れないようにするため。組織の作成者は引き継ぐ相手が居ないので、従来どおり
   全 Permission を持つ）
...
* 追加: 正常系（追加者の権限の引き継ぎを含む）/ 所属済みの重複追加
* 削除: 正常系 / 自分自身の削除拒否
