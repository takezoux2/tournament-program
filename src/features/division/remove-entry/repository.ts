 ...
 *   下限は形式ごとに決まる値なので、固定文言ではなく通知文言に使う
 *   minimum を一緒に返す。
 * - clearedOverCap: この形式の上限（maxEntries(current.format)）を残りエントリーが
 *   超えたままで、regenerateMatching が上限超過を理由に空を返した。
 *   上限は形式ごとに違う（トーナメントは 128 人、リーグは試合数が
 *   n(n-1)/2 で増えるため 16 人）。/edit で上限の緩い形式（トーナメント
 *   128 人）から上限の厳しいリーグ（16 人）へ切り替えた直後の部門でだけ
 *   起こりうる（生成は上限で守られているため、生成経由ではここに来る数の
 *   エントリーは作れない）。cleared と原因が違うので通知の文言も分ける。
 *   上限の人数は形式ごとに違うため、通知文言に使う limit を一緒に返す。
 */
...
export const removeEntryInDb: RemoveEntryPort = (ids, input) =>
  runDivisionSetup<RemoveEntryResult>(ids, async (_tx, current) => {
    ...
    // 形式の上限（リーグ）を超えているときも空を返すため、同じ「空になった」
    // でも原因が違えば運営者への伝え方を変える必要がある。
