 ...
 * 別の入口だから。画面の分岐だけでは、例えば編集画面を持たない形式の
 * 部門へ generateMatching を投げられると、どの画面にも出ない組み合わせが
 * matchingConfig に書き込まれてしまう。全スライスが必ず通るこの読み出しで
 * 弾いておけば、スライスごとに同じ判定を書き写す必要がなくなる
 * （現在の DivisionFormat は EDITABLE_FORMATS の 2 つしかないため実際には
 * 弾かれないが、形式が増えたときの安全策として残す）。
